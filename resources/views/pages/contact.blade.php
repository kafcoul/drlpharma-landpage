...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .contact-card { background: var(--gray-50); border: 1px solid var(--gray-100); border-radius: 20px; padding: 32px; text-align: center; transition: transform .3s, box-shadow .3s; }
        .contact-card:hover { transform: translateY(-4px); box-shadow: 0 12px 30px rgba(0,0,0,.08); }
        .contact-icon { width: 56px; height: 56px; background: var(--brand-100); border-radius: 16px; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; }
        .contact-icon svg { width: 28px; height: 28px; color: var(--brand-600); }
        .contact-card h3 { font-size: 18px; font-weight: 700; margin-bottom: 8px; }
        .contact-card p { font-size: 14px; color: var(--gray-500); margin-bottom: 12px; }
        .contact-card a.card-link { font-size: 15px; font-weight: 600; color: var(--brand-600); }
        .contact-card a.card-link:hover { color: var(--brand-700); }

        /* Form */
        .form-section { background: var(--gray-50); border-radius: 24px; padding: 48px 40px; border: 1px solid var(--gray-100); }
        .form-section h2 { font-size: 24px; font-weight: 800; margin-bottom: 8px; }
        .form-section > p { color: var(--gray-500); margin-bottom: 32px; }
        ...
        .form-group input, .form-group textarea, .form-group select {
            ...
            font-size: 15px; font-family: inherit; outline: none; transition: border-color .2s; background: #fff;
        }
        .form-group input:focus, .form-group textarea:focus, .form-group select:focus { border-color: var(--brand-500); }
        .form-group textarea { resize: vertical; min-height: 140px; }
        .form-submit { grid-column: 1 / -1; margin-top: 8px; }
        .btn-submit {
            display: inline-flex; align-items: center; gap: 10px;
            padding: 16px 32px; background: var(--brand-600); color: #fff;
            border: none; border-radius: 16px; font-weight: 700; font-size: 16px;
            cursor: pointer; transition: all .3s;
        }
        .btn-submit:hover { background: var(--brand-700); transform: translateY(-2px); box-shadow: 0 8px 20px rgba(5,150,105,.25); }
        .btn-submit svg { width: 20px; height: 20px; }

        /* Success */
        .form-success { display: none; text-align: center; padding: 40px 0; }
        .form-success.show { display: block; }
        .form-success .check-icon { width: 64px; height: 64px; background: var(--brand-100); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; }
        .form-success .check-icon svg { width: 32px; height: 32px; color: var(--brand-600); }
        ...
        <div class="container">
            ...
            <div class="contact-cards">
                ...
                <div class="contact-card">
                    ...
                    <h3>Adresse</h3>
                    <p>Siège social</p>
                    <span class="card-link" style="font-size:15px;font-weight:600;color:var(--brand-600);">Abidjan, Côte d'Ivoire</span>
                </div>
            </div>
